...
/**
 * Main class to house all factory (static) methods to instantiate parallel {@link java.util.stream.Collector}s
 *
...
public final class AsyncCollectors {
     ...
     * @param <R>       type of the result given by {@code mapper}
     * @param <RR>      type of the result captured in {@code CompletableFuture}
    ...
    public static <T, R, RR> Collector<T, ?, CompletableFuture<RR>> parallel(Function<? super T, ? extends R> mapper,
        ...
        return AsyncCollectorsFactory.collecting(i -> i.collect(collector), mapper);
    }
